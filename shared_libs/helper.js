...
// Find web element by name with Js
Helper.prototype.findElementByNameJs = function (elementType, nameAttribute) {
  var ele = element(by.js(function() {
    var inputs = document.querySelectorAll(elementType)
    for (var i = 0; i < inputs.length; i++) {
      if (inputs[i].name.localeCompare(nameAttribute)) {
        return inputs[i]
      }
    }
  }))
  this.elementIsPresenceDom(ele)
  return ele
}
